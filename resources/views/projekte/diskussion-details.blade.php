...
<div class="max-w-2xl mx-auto px-4 mt-6 mb-16">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5 mb-4">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 flex-wrap">
            <div
                class="w-7 h-7 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center text-xs font-semibold uppercase flex-shrink-0">
    ...
    <div x-data="{ antwortFormular: false, editMode: false, menuOpen: false }"
        class="bg-white border border-gray-200 rounded-xl p-4 mb-3">
        <div class="flex items-center gap-2 mb-3 flex-wrap">
            <div
                class="w-8 h-8 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center text-xs font-semibold uppercase flex-shrink-0">
            ...
            <div class="relative ml-2">
                ...
                <div x-show="menuOpen" @click.away="menuOpen = false" style="display: none;"
                    ...
                    <form action="{{ route('beitrag.loeschen', $antwort->id) }}" method="POST"
        ...
        <div x-show="editMode" x-collapse>
            <form action="{{ route('diskussion.bearbeiten', $antwort->id) }}" method="POST" class="mb-2">
                @csrf @method('PUT')
                <textarea name="content"
                    class="w-full text-sm border-gray-200 rounded-lg">{{ $antwort->content }}</textarea>
                <div class="flex justify-end gap-2 mt-1">
                    <button type="button" @click="editMode = false"
                        class="text-xs text-gray-500 hover:bg-gray-100 px-2 py-1 rounded">Abbrechen</button>
                    <button type="submit"
                        class="text-xs bg-[#6ba9dc] hover:bg-[#5a91c4] text-white px-2 py-1 rounded">Speichern</button>
                </div>
            </form>
        </div>
        ...
        <button @click="antwortFormular = !antwortFormular"
            class="mt-2 text-xs font-medium text-gray-500 hover:text-blue-600 transition-colors flex items-center gap-1">
        ...
        <div x-show="antwortFormular" x-collapse class="mt-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
            <form action="{{ route('antwort.speichern', $antwort->id) }}" method="POST">
                @csrf
                <textarea name="content" rows="2" required placeholder="Schreibe eine Antwort..."
                    class="w-full text-sm border-gray-200 rounded-lg"></textarea>
                <div class="flex justify-end gap-2 mt-2">
                    <button type="button" @click="antwortFormular = false"
                        class="text-xs text-gray-500 px-2 py-1 hover:bg-gray-200 rounded">Abbrechen</button>
                    <button type="submit"
                        class="text-xs bg-[#6ba9dc] hover:bg-[#5a91c4] text-white px-3 py-1 rounded">Senden</button>
                </div>
            </form>
        </div>
        ...
        <div class="mt-4 pl-4 border-l-2 border-blue-100 space-y-3">
            ...
            <div x-data="{ replyFormular: false, editKindMode: false, menuOpen: false }"
                class="bg-gray-50 p-3 rounded-lg border border-gray-100">
                {{-- Kopf der Unterantwort --}}
                <div class="flex justify-between items-center mb-1">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-[10px] font-bold text-gray-700 uppercase">{{ $unter->ersteller->name ?? 'NA' }}</span>
                    ...
                    <div class="relative ml-2">
                        ...
                        <div x-show="menuOpen" @click.away="menuOpen = false" style="display: none;"
                            ...
                            <form action="{{ route('beitrag.loeschen', $unter->id) }}" method="POST"
                ...
                <div x-show="editKindMode" x-collapse>
                    <form action="{{ route('diskussion.bearbeiten', $unter->id) }}" method="POST" class="mb-1">
                        @csrf @method('PUT')
                        <textarea name="content"
                            class="w-full text-xs border-gray-200 rounded">{{ $unter->content }}</textarea>
                        <div class="flex justify-end gap-1 mt-1">
                            <button type="button" @click="editKindMode = false"
                                class="text-[9px] text-gray-500 hover:bg-gray-200 px-2 py-1 rounded">Abbrechen</button>
                            <button type="submit"
                                class="text-[9px] bg-[#6ba9dc] hover:bg-[#5a91c4] text-white px-2 py-0.5 rounded">Speichern</button>
                        </div>
                    </form>
                </div>
                ...
                <button @click="replyFormular = !replyFormular"
                    class="mt-2 text-[10px] font-medium text-gray-500 hover:text-blue-600 transition-colors flex items-center gap-1">
                ...
                <div x-show="replyFormular" x-collapse class="mt-2 bg-gray-100 p-2 rounded-lg border border-gray-200">
                    <form action="{{ route('antwort.speichern', $unter->id) }}" method="POST">
                        @csrf
                        <textarea name="content" rows="2" required placeholder="Antworte auf diesen Kommentar..."
                            class="w-full text-xs border-gray-200 rounded-lg"></textarea>
                        <div class="flex justify-end gap-2 mt-1">
                            <button type="button" @click="replyFormular = false"
                                class="text-[9px] text-gray-500 px-2 py-1 hover:bg-gray-200 rounded">Abbrechen</button>
                            <button type="submit"
                                class="text-[9px]bg-[#6ba9dc] hover:bg-[#5a91c4] text-white px-2 py-0.5 rounded">Senden</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 mt-2">
        ...
        <form action="{{ route('beitrag.speichern', $diskussion->id) }}" method="POST">
            @csrf
            <textarea name="content" placeholder="Deine Antwort..." rows="3"
