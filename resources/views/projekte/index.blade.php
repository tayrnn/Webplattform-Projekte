...
<div class="flex items-center gap-4 mb-6 flex-wrap">
    ...
    <div class="flex gap-1 border-b border-gray-200">
        <a href="{{ route('projekte.index') }}" class="px-4 py-2 text-sm font-medium border-b-2 transition-colors
                  {{ !request()->routeIs('projekte.meine')
                       ? 'border-blue-600 text-blue-600'
                       : 'border-transparent text-gray-500 hover:text-gray-700' }}">
            Alle Ideen
        </a>
        <a href="{{ route('projekte.meine') }}" class="px-4 py-2 text-sm font-medium border-b-2 transition-colors
                  {{ request()->routeIs('projekte.meine')
                       ? 'border-blue-600 text-blue-600'
                       : 'border-transparent text-gray-500 hover:text-gray-700' }}">
            Meine Projekte
    ...
    <div class="ml-auto flex items-center gap-3">
        ...
        <div class="relative" x-data="{ offen: false }" @click.outside="offen = false">
            ...
            <button @click="offen = !offen" type="button" class="flex items-center gap-2 px-4 py-2 text-sm border rounded-lg bg-white
                ...
                @endif
                <svg class="w-3 h-3 transition-transform" :style="offen ? 'transform:rotate(180deg)' : ''" fill="none"
            ...
            <div x-show="offen" x-transition:enter="transition ease-out duration-150"
                ...
                x-cloak class="absolute right-0 top-[calc(100%+8px)] z-50 w-64 bg-white
                ...
                <form method="GET" action="{{ url()->current() }}" id="filter-form">
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-100 text-center">
                        <a href="{{ url()->current() }}" class="text-sm text-blue-600 hover:underline">
                            Filter zurücksetzen
                        </a>
                    </div>
                    @endif
        ...
        {{-- Neue Idee --}}
        <a href="{{ route('projekte.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium
